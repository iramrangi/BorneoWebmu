...
function initializeScrollEffects() {
  ...
  const animateElements = document.querySelectorAll(
    '.service-card, .package-card, .section-header, .contact-item, .team-member, .section-title'
  );
  ...
  faqToggles.forEach(toggle => {
    toggle.addEventListener('click', () => {
      ...
      if (isExpanded) {
        ...
        toggle.querySelector('.faq-icon').style.transform = 'rotate(0deg)';
      } else {
        // Expand
        toggle.setAttribute('aria-expanded', 'true');
        faqAnswer.setAttribute('aria-hidden', 'false');
        faqAnswer.style.maxHeight = faqAnswer.scrollHeight + 'px';
        toggle.querySelector('.faq-icon').style.transform = 'rotate(180deg)';
      }
    });
